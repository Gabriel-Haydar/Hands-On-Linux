...
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff) {
    ...
    if (strcmp(attr_name, "led") == 0) {
        ...
        strcpy(command, "GET_TEMP");
    } else if (strcmp(attr_name, "hum") == 0) {
        strcpy(command, "GET_HUM");
    } else {
        return -EINVAL;
    }
...
static ssize_t attr_store(struct kobject *sys_obj, struct kobj_attribute *attr, const char *buff, size_t count) {
    ...
    if (strcmp(attr_name, "led") == 0) {
        snprintf(command, sizeof(command), "SET_LED %ld", value);
        usb_send_cmd(command);
        msleep(50);
    } else if (strcmp(attr_name, "ldr") == 0) {
        printk(KERN_ALERT "SmartLamp: Nao e possivel setar o valor do LDR.\n");
        return -EACCES;
    } else if (strcmp(attr_name, "temp") == 0) {
        printk(KERN_ALERT "SmartLamp: Nao e possivel setar o valor da temperatura.\n");
        return -EACCES;
    } else if (strcmp(attr_name, "hum") == 0) {
        printk(KERN_ALERT "SmartLamp: Nao e possivel setar o valor da humidade.\n");
        return -EACCES;
    } else {
        return -EINVAL;
    }
